---
name: metrics-tracking-knowledge
description: >
  Product metrics hierarchy (North Star, L1, L2), OKR goal setting, DAU/WAU/MAU,
  retention, conversion, activation metrics, dashboard design, review cadences,
  experimentation. Auto-activates when setting goals and targets, building dashboards,
  running metrics reviews, tracking KPIs, or diagnosing metric movements.
---

# Metrics Tracking Knowledge
...
- Which timeframe matters most? DAU for daily-use products. WAU for weekly-use products. MAU for less frequent products.
...
**Common timeframes**: D1 (next day), D7 (one week), D30 (one month), D90 (three months)
...
- Segment retention by activation behavior. Users who completed onboarding vs those who did not.

### Conversion
**What it measures**: % of users who move from one stage to the next.

**Common funnels**: Visitor to signup → signup to activation → free to paid → trial to paid → monthly to annual
...
**Defining activation**:
- Look at retained vs churned users. What actions did retained users take that churned users did not?
...
- Key Results should measure outcomes (user behavior, business results), not outputs (features shipped).
...
4. **Fewer metrics, more insight**. A dashboard with 50 metrics helps no one. Focus on 5-10 that matter.

5. **Consistent time periods**. Use the same time period for all metrics on a dashboard.

6. **Visual status indicators**. Use color to indicate health at a glance: Green (on track), Yellow (needs attention), Red (off track or declining).
...
**Third row**: Key funnels or conversion metrics.
**Fourth row**: Recent experiments and launches with early metrics.
**Bottom / drill-down**: L2 metrics, segment breakdowns, detailed time series.

### Dashboard Anti-Patterns
- **Vanity metrics**: Metrics that always go up but do not indicate health (total signups ever, total page views)
- **Too many metrics**: If it does not fit on one screen, cut metrics.
...
- **Output dashboards**: Measuring team activity (tickets closed) instead of user and business outcomes

### Alerting
Set alerts for metrics that require immediate attention:

- **Threshold alerts**: Metric drops below or rises above a critical threshold
